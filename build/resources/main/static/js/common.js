...
$(document).ready(function(){
	...
	// 채팅창 수행일지 리스트 더보기
	$('.record-list-wrap .btn-more').on('click', function(e) {
		var $record_wrap = $(this).parents().find('.record-list-wrap');
	...
	$('.btn-more-all').on('click', function(e) {
		debugger
		var $record_wrap = $(this).parents().find('.record-list-wrap');
		$record_wrap.toggleClass('on-all');
		$(this).toggleClass('on');
	...
	$(self.$el).datepicker({
		...
		onClose: function( selectedDate ) {    
			...
			if($(".hasDatepicker")[0].id == this.id) {
				$("#"+ $(".hasDatepicker")[1].id +"").datepicker( "option", "minDate", selectedDate );
			} else {
				$("#"+ $(".hasDatepicker")[0].id +"").datepicker( "option", "maxDate", selectedDate );
			}
		},
//          ,minDate: new Date(),
		monthNames: ['1월', '2월', '3월', '4월', '5월', '6월', '7월', '8월', '9월', '10월', '11월', '12월'],
	...
	axiosCall('post', '/api/commDtlCodeAllList', codeParamList, function(res) {
		...
		if(res.data.length == 0) {
			...
			$.each(res.data, function(idx, obj) {
				...
				// 전체 or 선택
				var comboType = codeParamList[idx].comboType;
				...
				self[codeParamList[idx].arr_nm] = codeDtlList;
...
// 공통 confirm
function cmmConfirm(val, callback) {
	// alert창 띄우기
	$("#alertPopup").show();
